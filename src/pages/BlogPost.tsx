...
export default function BlogPost() {
  ...
  // Safely parse title and content
  const title = new DOMParser().parseFromString(post.title.rendered, 'text/html').body.textContent || post.title.rendered;
  ...
  return (
    <div className="pt-24 pb-24 bg-white">
      <SEO 
        title={`${title} - Hongyuan Precision`} 
        description={post.excerpt?.rendered?.replace(/<[^>]+>/g, '').slice(0, 150) || "Read about precision manufacturing, cnc machining and insights"}
      />
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="mb-10">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 leading-tight mb-6" dangerouslySetInnerHTML={{ __html: post.title.rendered }} />
